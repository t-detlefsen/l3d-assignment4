...
def colours_from_spherical_harmonics(spherical_harmonics, gaussian_dirs):
    ...
    x = gaussian_dirs[:, 0].unsqueeze(1).repeat(1, 3)
    y = gaussian_dirs[:, 1].unsqueeze(1).repeat(1, 3)
    z = gaussian_dirs[:, 2].unsqueeze(1).repeat(1, 3)
    ...
    colours = SH_C0 * c0
    colours += -SH_C1 * y * c1 + SH_C1 * z * c2 - SH_C1 * x * c3
    colours += SH_C2_0 * xy * c4 + \
               SH_C2_1 * yz * c5 + \
               SH_C2_2 * (2.0 * zz - xx - yy) * c6 + \
               SH_C2_3 * xz * c7 + \
               SH_C2_4 * (xx - yy) * c8
    colours += SH_C3_0 * y * (3.0 * xx - yy) * c9 + \
               SH_C3_1 * xy * z * c10 + \
